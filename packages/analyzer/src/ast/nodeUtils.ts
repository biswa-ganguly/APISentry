import { Node, SyntaxKind, ObjectLiteralExpression, PropertyAssignment, ShorthandPropertyAssignment } from 'ts-morph';
import { CodeLocation, ContractField, PrimitiveContractType } from '@apisentry/types';
import { normalizePath, normalizeRoutePattern } from '@apisentry/shared';
...
export function inferTypeAndChildrenFromNode(node: Node | undefined): {
  ...
  // Fallback to type checker if ts-morph has static type info
  try {
    ...
    // Ignore type-checker errors on transient AST nodes
  }
